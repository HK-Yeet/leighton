...
module.exports = {
    name: "help",
    aliases: ["commands"],
    description: 'Get some information about this bot\'s commands!',
    category: "utilities",
    hidden: true,
    ...
    callback: async (bot, message, args, hkandler) => {
        if (!args.length) {
            ...
            let pages = Array.from(categories.map((category, name) => category.map((command) => `\n\n**${command.name}**\nAliases: [ ${command.aliases ? command.aliases.join(', ') : 'None'} ]\nDescription: *${command.description ? command.description : 'N/A'}*`)));
            let page = 0;
            let embed = {
                color: '60B29B',
            ...
            msg.react("◀️");
            msg.react("<a:garbage:810246427427078235>");
            msg.react("▶️");
